...
def begin_crawl(asin, keyword, country_host, country_code, retries, output):
    returnDictionary = {}
    ...
    if page == None:
        ...
        sleep(2)
        if (retries < 3):
            return begin_crawl(asin, marketplace, keyword, retries + 1, output)
...
def parallel_crawl(asin, keywords, country_host, country_code):
    ...
    processes = [mp.Process(target=begin_crawl, args=(asin, keyword, country_host, country_code, 0, output_queue)) for keyword in product.keywords]
    #intial_time = time()
    # Run processes
    for p in processes:
        p.start()
